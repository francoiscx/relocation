...
if(isset($_POST['signupBtn'])){
    ...
    //echo $RegisteredBy . "<br>";
            ...
            $tempPassword = Random::AlphaNumeric(10);
            //$tempPassword = substr($tempPassword, -3, 25); // returns "d"
            //$tempPassword = String2Stars($tempPassword,5,-5);
    ...
    $RFirstname = name_field($RFirstname);
		...
		$REmail = strtolower($REmail);
        
    try{    
    
        $sqlInsert = "INSERT INTO users (user_email, password, temp_password, user_first_name, user_last_name, registration_date, registered_by)
                      VALUES (:user_email, :password, :temp_password, :user_first_name, :user_last_name, now(), :registered_by)";
                      
        $statement = $db->Prepare($sqlInsert);
        $statement->execute(array(':user_email' => $REmail, ':password' => $hashed_password, ':temp_password' => $tempPassword, ':user_first_name' => $RFirstname, ':user_last_name' => $RLastname, ':registered_by' => $RegisteredBy));
